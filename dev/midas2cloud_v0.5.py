...
#            file_in_dir=sorted(os.listdir(INAPATH))
            ...
            files = (
               ...
               if entry.is_file() and entry.name.startswith("run") and entry.name.endswith(".mid.gz")
            )
...
#                    cy.daq_read_runlog_replica_status(connection, runN, storage="local", verbose=verbose)==0: 
                        ...
                        while(not status):   
                            ...
                            if filesize < 5400000000: # ~ 5 GB
                                ...
                                docker_command = [ "docker", "run", "--rm",
                                                      ...
                                                      "gmazzitelli/rucio-uploader:v0.2",  # nome dell'immagine Docker
                                                      "--file", "/app/"+filename,
                                                      "--bucket", bucket,
				...
				#| 4         | Client configuration error                         |
                                ...
                                if (exit_code==0 or exit_code==1):
                                    ...
                                    if verbose: print('[DEBUG] Exit code: '+str(exit_code))
                                    ...
                                    print('{:s} WARNING: Uploading file: {:s}, try: {:s}, code error: {:s}'.format(dtime, filename, str(current_try), str(exit_code)))
                                    cy.daq_update_runlog_replica_status(connection, 
                                                                            runN, storage="cloud", 
                                                                            status=-2, verbose=verbose)
                                    cy.daq_update_runlog_replica_status(connection, runN, 
                                                        storage="tape", status=-2, verbose=verbose)
                                    if current_try==max_tries:
                                        print('{:s} ERROR: Max try number reached'.format(dtime))
                                        print("DEBUG result: ",result.returncode, result.stdout, result.stderr)
                                        status = True
                            else:
