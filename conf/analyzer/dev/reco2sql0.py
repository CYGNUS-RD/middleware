...
def main(verbose=False):
    ...
    file_out_path = '/jupyter-workspace/cloud-storage/cygno-analysis/pkl/'
    ...
    sqlRec = "SELECT * FROM `SlowReco` ORDER BY `run_mean` DESC  LIMIT "+str(sql_limit)+";"
    ...
    for i, run in enumerate(sql_Log.run_number):
        if force_rebuild or not (run in sql_Rec.run_mean.astype(int).tolist()):
            print("analyzing run: ",run, str(sql_Log[sql_Log.run_number==run].run_description.values))
            try:
                branch_data = {}
