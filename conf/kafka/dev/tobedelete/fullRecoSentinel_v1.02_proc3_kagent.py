...
def main(run_number_start, run_number_end, nproc, outname='df_condor', just_status=False, verbose=False):
    ...
    #for j in [5629, 5628, 5627, 5626, 5625, 5624, 5623, 5616, 5615, 5614, 5613, 5612, 5611, 5602, 5605]:
    #    df_condor.loc[df_condor['Cluster_ID'] == j, 'Cloud_storage'] = 1
    
    #df_condor.drop(df_condor[df_condor['Cluster_ID'] == 5652].index, inplace = True)
...
if __name__ == "__main__":
    ...
    if len(args) < 1:
        ...
        main(int(args[0]), options.run_number_end, options.nproc, options.outname, options.just_status, options.verbose)

        ## Example:
        # ./fullRecoSentinel_v1.02.py 17182 -e 20000 -j 3 -o df_condor_coda1 -s -v
