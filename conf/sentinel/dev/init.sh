...
python3 -u /root/dev/fullRecoSentinel_v1.02.py ${STARTRUN} -e ${ENDRUN} -j ${NCORE} -i ${MAXIDLE} -t ${TAG} -f ${RECOPATH} -o ${DFPANDAS} -s ${DRAIN} -v >> /root/dev/log/reco${DFPANDAS}.log  2>&1
