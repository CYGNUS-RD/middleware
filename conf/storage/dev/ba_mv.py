    ...
    try:
        while IsTruncated:
            ...
            for i, file in enumerate(contents):
               ...
               if ((bucket.name=='cygno-analysis') and  ('plk' in file['Key'])):
                   new_file_key = 'pkl/'+file['Key'].split('/')[1]
                   old_file_key = file['Key']
                   print("moving: ", old_file_key, new_file_key)
                   s3r.Object(bucket.name,new_file_key).copy_from(CopySource=bucket.name+'/'+old_file_key)
                   s3r.Object(bucket.name,old_file_key).delete()
            if IsTruncated:
