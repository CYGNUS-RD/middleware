...
compress_and_replace() {
    ...
    # ✅ Resume check
    if rclone lsf "${REMOTE}:${remote_parent}" | grep -q "^${archive_name}$"; then
        echo "⏭️ Archivio già presente, salto: ${remote_dir}"
        return
    fi
...
create_tar_batch() {
    ...
    echo "Controllo resume..."
    if rclone lsf "${REMOTE}:${remote_dir}" | grep -q "^${archive_name}$"; then
        echo "⏭️ Batch archivio già presente, salto"
        return
    fi
...
# Le funzioni pack_small_files_in_dir, process_dir e verify_no_small_leftovers restano INVARIATE
# Inseriscile come nel tuo script precedente.
